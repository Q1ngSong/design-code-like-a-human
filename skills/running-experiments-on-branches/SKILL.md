...
**调参和实验不在主分支上跑；收尾用 `git merge --no-ff` 保留整条实验历史。**
主分支通过 `git log --first-parent` 浏览每组实验的合并节点，
通过 `git log --graph --oneline --all` 查看具体尝试。默认不 squash、不 rebase、不 cherry-pick 代替合并。
...
- **结论为空**：记录尚未完成。尚未启动的计划先核对是否仍在授权范围内，暂停或取消的不自动重跑。
...
**运行完成、记录完成、本地合并完成、推送完成是不同状态。** 用户要求完成整组实验或收尾，
...
不 reset 删除先前尝试。只暂存本组路径，审阅 `git diff --cached` 后提交；说明保留内容和理由。
...
下列步骤对每一级目标分别执行。示例变量来自开组记录；恢复会话时重新读记录，不能依赖旧 shell 变量。
...
通过 `git log --graph --oneline --all` 查看完整历史。**实验分支默认保留**，已提交的失败记录
和尝试也成为目标分支的祖先；stash 中的失败代码仍然只在本地。
...
本地合并和远端同步分别处理。用户已经授权向指定远端推送指定分支时，完成验证后直接执行；
否则先完成本地可审阅结果，需要发布时再询问。示例 `git push origin main develop` 仅适用于
...
只移除已核对的 worktree，不使用 `--force` 掩盖未保存状态；实验分支继续保留。
