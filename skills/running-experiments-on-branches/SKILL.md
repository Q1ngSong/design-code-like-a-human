...
这组要比较两种以上方案（新想法对旧方案），要训练或调参时，开组先问用户要不要自动调参、每方案调几次（只评测已冻结的方法不问，见 `evaluating-frozen-methods`），
...
- **去重**：追加记录前，用 CSV 读取器在本组文件中精确匹配 `task` 字段，避免将名称中的
  ...
  不要仅换个名字重跑已经否定的尝试。
- **资源不足**：磁盘空间满、内存不足但未触发 OOM。先清理输出目录中的中间文件，
  释放空间后重试。仍失败时按 crash 处理，结论注明资源限制。
...
下一轮的 agent 会把同一条死路再走一遍。

代码那边同步处置一下。放弃的如果是一整种做法，而它已经按 `writing-minimal-code`
「换一种做法」放在自己的文件里，就看还要不要拿它当对照：还要就留着，角色标记改成 `[旁支]`；
不要了才删掉那个文件和字典里那一行。分支留不留是另一回事，见「合并：保留两个父提交」。
