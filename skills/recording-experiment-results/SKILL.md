...
description: Use when planning, resuming, finishing, or summarizing experiments - record each task's plan before running, then its metrics, conclusion, output path, and reproducible version references before moving on. 触发场景:实验计划、实验恢复、实验跑完了、记录结果、汇总结果、这组实验的结论、上次那个实验结果呢、整理结论、更新总览。
---
...
| `简介` | **为什么试这个**，不是「跑了个实验」 |
| `指标` | 你关注的那几个数，自由格式（`FID 12.3 / CLIP 0.281`）；对比实验还要标明在正式测试集上跑满了，写法见下一节「对比实验」 |
...
要比较两种以上方案时（新想法对旧方案、我们的方法对 baseline），开跑前按 `comparing-methods-fairly`
在组 README 里冻结对照协议。协议写好后，记录要守它的格式，本 skill 的脚本会检查：

- `简介` 以对照方的名字开头，如 `old: 新条件下重训`。
- keep 和 discard 的 `指标` 标明正式测试集跑满了：`FID 12.3 @data/coco5k 1000/1000`，没写 N 的写 `@data/coco5k 全部`。
- crash 和 timeout 的 `指标` 填 `—`，跑了一部分的数字留在日志里。
- 写 summary 前每一方都要有 keep 或 discard；各方全部 crash 或 timeout 时，summary 的 `指标` 填 `—`，
  写明没形成有效对比、不判胜负。写完用 `exp-check` 查。
